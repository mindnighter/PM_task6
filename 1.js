...
let regPhone = /\(?\d{3}\)?\s\d{3}\-\d{2}\-\d{2}/;

for(let elem in hash){
    var shadow = Symbol(elem);
    Object.defineProperty(hash[elem], "id", {
        writable:false,
        configurable: false
    });
    Object.defineProperty(hash[elem], 'phoneSet', {
        set: function(newValue) { if(regPhone.test(newValue))this.phone = newValue },
      });
}
...
document.getElementById('3').addEventListener('click',()=>{
    let com = prompt('Введите название компании');
    let arr = [];
    for(let elem in hash){
        if(com ==  hash[elem].company.name){
            arr.push(hash[elem]);
        }
    }
    if(arr[0]){
        ...
        console.log('Пользователя с такой компании нет');
    }
